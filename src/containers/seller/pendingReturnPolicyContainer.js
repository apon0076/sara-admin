...
class pendingReturnPolicyContainer extends Component {
  constructor(props) {
    super(props)
    this.state = {
      pendingReturn: null,
  ...
  componentDidMount = async () => {
    ...
    await this.props.getPendingSellerReturnPolicyRecord()
    this.setState({
      pendingReturn: this.props.pendingReturnPolicy.sort((a, b) =>
        a.timeM > b.timeM ? 1 : -1
      ),
    })
  }
  ...
  activeReturnPolicy = async (e) => {
    ...
    const data = {
      sellerReturnPolicyId: this.state.pendingReturn[0].sellerReturnPolicyId,
      duration: this.state.pendingReturn[0].duration,
      returnPolicy: this.state.pendingReturn[0].returnPolicy,
      shopId: this.state.pendingReturn[0].shopId,
      sellerId: this.state.pendingReturn[0].sellerId,
      isActive: 'Y',
      isApprove: this.state.pendingReturn[0].isApprove,
    }
                    ...
                    <div className='card'>
                      ...
                      {this.props.loading ? (
                        ...
                        <DataTable
                          header={header}
                          ref={(el) => (this.dt = el)}
                          value={this.state?.pendingReturn}
                          className='p-datatable-customers p-datatable-responsive-demo'
                          dataKey='pendingReturnPolicyId'
                          rowHover
                          globalFilter={this.state.globalFilter}
                          paginator
                          rows={10}
                          emptyMessage='No pending Return Policy Found!'
...
const mapStateToProps = (state) => ({
  pendingReturnPolicy: state.sellerReducer.pendingReturnPolicy,
...
const mapDispatchToProps = (dispatch) => {
  return {
    getPendingSellerReturnPolicyRecord: () =>
      dispatch(sellerAction.getPendingSellerReturnPolicyRecord()),
