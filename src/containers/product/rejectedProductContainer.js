...
class rejectedProductContainer extends Component {
  constructor(props) {
    super(props)
    this.state = {
      products: null,
  ...
  componentDidMount = async () => {
    ...
    if (roleId === '1') {
      this.setState({
        ...
        //shopId: this.props.sellerProfile.sellerprofileById.shopId,
      })
    } else {
      this.setState({
        authenticated: false,
    ...
    await this.props.getRejectedProductRecord()
    this.setState({
  ...
  setApprove = async (seller) => {
    ...
    if (res.type === 'APPROVE_SHOP_ERROR') {
...
const mapStateToProps = (state) => ({
  products: state.productReducer.rejectedProducts,
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    getRejectedProductRecord: () =>
      dispatch(productAction.getRejectedProductsRecord()),
