...
import CreateProductSellerTest from "../../components/product/CreateProductSellerTest";
...
class CreateProductContainerTest extends Component {
  fileObj = [];
  fileArray = [];
  constructor(props) {
    super(props);
  ...
  componentDidMount = async () => {
    ...
    await this.props.getApprovedReturnPolicyByShopIdRecord(this.state.shopId);
    this.setState({
      returnDuration: this.props?.approvedReturnPolicyById[0]?.duration || 0,
    });
  };
    ...
    if (result && result?.payload?.success?.data?.succeed === true) {
      if (result.type === "CREATE_PRODUCT_SUCCESS") {
        ...
        setTimeout(() => {
          ...
          this.props.history.push("/manage-seller-products?status=approved");
        }, 2500);
      } else {
        toast.error("Something went wrong, Please try again");
        setTimeout(() => {
          this.props.history.push("createProductSellerTest");
        }, 2500);
      }
