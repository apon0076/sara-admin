  ...
  componentDidMount = async () => {
    ...
    if (roleId === '1') {
      this.setState({
        ...
        isReturnable:
          this.props?.location?.state?.rowData?.isReturnable === 'Y' ? true : false,
        showOnHomepage:
          this.props?.location?.state?.rowData?.showOnHomepage === 'Y'
            ? true
            : false,
        isProduct:
          this.props?.location?.state?.rowData?.isProduct === 'Y' ? true : false,
        ...
        parentCategoryId: this.props?.location?.state?.rowData?.parentCategoryId,

        pictureId: this.props?.location?.state?.rowData?.pictureId,
        productImagePath: this.props?.location?.state?.rowData?.productImagePath,
      })
    } else {
      this.setState({
        authenticated: false,
