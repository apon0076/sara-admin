...
import EditCourierCost from '../../components/shipping/EditCourierCost'

class editCourierCostContainer extends Component {
  constructor(props) {
    super(props)
    this.state = {
      courierCostId: 0,
  ...
  componentDidMount = async () => {
    ...
    if (roleId === '1') {
      this.setState({
        ...
        courierCostId: this.props.location.state.rowData.courierCostId,
        courierId: this.props.location.state.rowData.courierId,
        customsDutiesChargePer: this.props.location.state.rowData.customsDutiesChargePer,

        taxPer: this.props.location.state.rowData.taxPer,
        vatPer: this.props.location.state.rowData.vatPer,
        aitPer: this.props.location.state.rowData.aitPer,
        fuelSurchargePer: this.props.location.state.rowData.fuelSurchargePer,
        otherCost: this.props.location.state.rowData.otherCost,
    ...
    switch (target.name) {
      case 'courierName':
        this.setState({
            ...
            errorCourierId: target.value.length < 1 ? "Select one Courier Name" : ""
        })
        break
  ...
  saveCourierCost = async (e) => {
    ...
    if (result && result.payload.success.succeed === true) {
      if (result.type === 'CREATE_OR_UPDATE_COURIER_COST_SUCCESS') {
        toast.success('Courier Cost Updated Successfully')
        setTimeout(() => {
        ...
        setTimeout(() => {
          this.resetForm()
          this.props.history.push('EditCourierCost')
        }, 2500)
      }
