...
import CreateCourierProfile from '../../components/shipping/CreateCourierProfile'
import Resizer from 'react-image-file-resizer';

class createCourierProfileContainer extends Component {
  constructor(props) {
    super(props)
    this.state = {
      courierId: 0,
  ...
  componentDidMount = async () => {
    ...
    this.setState({
      countryList: this.props.allCountries.map(({ countryName: label, countryId: value }) => ({
        label,
        value
      }))
    })
  ...
  handleAddressChange = async (field, e) => {
    switch (field) {
      case 'countryList':
        ...
        this.setState({
          cityList: this.props.allCities.map(({ cityName: label, cityId: value }) => ({
            label,
            value
          }))
        })
      ...
      case 'cityList':
        ...
        this.setState({
          areaList: this.props.allAreas.map(({ areaName: label, areaId: value }) => ({
            label,
            value
          }))
        })
  ...
  saveCourierProfile = async (e) => {
    ...
    if (result && result.payload.success.succeed === true) {
      if (result.type === "CREATE_OR_UPDATE_COURIER_PROFILE_SUCCESS") {
        toast.success("Courier Profile Created Successfully")
        setTimeout(() => {
