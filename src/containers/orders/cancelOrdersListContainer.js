...
import { fetchCancelOrdersRecord } from '../../store/actions/orderAction'
...
export default function CancelOrdersListContainer() {
  ...
  useEffect(() => {
    dispatch(fetchCancelOrdersRecord(reformed_status, currentPage, itemPerPage))
  ...
  useEffect(() => {
    if (location.location.pathname === '/CancelOrders')
