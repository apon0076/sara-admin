...
const FlatRateProductCategory = (props) => {
  ...
  const getCategories =
    shippingCostData &&
    shippingCostData.categories.length > 0 &&
    shippingCostData.categories.map((obj) => ({
  ...
  const header = (
    <div className='table-header'>
      <h5 className='p-mx-0 p-my-1'>Category Lists</h5>
  ...
  const textEditor = (options) => {
    return (
      <InputNumber
        value={options.value < 0 ? 0 : options.value}
        onValueChange={(e) => options.editorCallback(e.value < 0 ? 0 : e.value)}
      />
    )
  }
  ...
  const onCategoryRowEditComplete = (e) => {
    ...
    let { newData, index } = e
    _selectedCategories[index] = newData
    setSelectedCategories(_selectedCategories)
    shippingCostData.getProductCategoryHandler(_selectedCategories)
  }

  const saveShippingCostFlatRateProductCategory = async (e) => {
    e.preventDefault()
    ...
    <div
      style={{
        display:
          ...
          shippingCostData.productCategorySelected === false
            ? 'none'
                  ...
                  <div className='text-center'>
                    <div className='btn-group text-center'>
                      <button
                        ...
                        onClick={saveShippingCostFlatRateProductCategory}
                      >
                        Create
                      </button>
                      <Link to='/Home'>
...
export default FlatRateProductCategory
