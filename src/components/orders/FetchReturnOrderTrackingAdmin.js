...
export default function FetchReturnOrderTrackingAdmin({
  orderInfo,
  setVisible,
  counts,
  setCounts,
  ...
  const handleChange = (e) => {
    ...
    switch (select.name) {
      case 'statusTypeName':
        setStatusId(select?.value?.orderStatusTypeId)
  ...
  const return_order_status = useSelector(
    (state) => state.orderReducer.postReturnOrder
  )
  const loading = useSelector((state) => state.orderReducer.loading)
  useEffect(() => {
    if (return_order_status?.succeed === true) {
      setCounts(counts + 1)
      setVisible(false)
